...
public class ClienteDAOPostgreSQL extends ControladorAbstract implements ClienteDAO {

	public ClienteDAOPostgreSQL() {
		super();		
	}

	public ClienteDAOPostgreSQL(Connection connection) {
		super(connection);
	}
...
//			throw new DAOException("Errro al guardar cliente", e);
	...
	public void borrar(Long codigo) {
		...
		try {
			String sql = String.format(
					"delete from  cliente where codigo=%d",
					codigo);			
...
//			throw new DAOException("Error buscando cliente", e);
	...
	public List<Cliente> buscarPorCampo(String campo, String valor) {
		...
		try {
			String sql = String.format(
					"select * from cliente where %s ilike '%s'", campo,("%"+valor+"%"));			
			PreparedStatement ps = this.connection.prepareStatement(sql);
